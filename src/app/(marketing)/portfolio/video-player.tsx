...
const VideoPlayer = React.forwardRef<HTMLVideoElement, VideoPlayerProps>(
  (
    ...
    const handleProgressClick = (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      seekToTime(
        clamp(Math.round((percent * duration) / 0.01) * 0.01, 0, duration),
      );
    };
    ...
    const handleMouseMove = () => setShowControlsState(true);
    const handleMouseLeave = () => {
      if (autoHide) setShowControlsState(false);
    };
    ...
    React.useEffect(() => {
      ...
      syncEvents.forEach((e) => video.addEventListener(e, hardResync));
      ...
      return () => {
        syncEvents.forEach((e) => video.removeEventListener(e, hardResync));
    ...
    React.useEffect(() => {
      const handleVisibilityOrFocus = () => hardResync();
    ...
    return (
      ...
      >
        ...
        {/* Persistent play button — visible whenever paused */}
        {!isPlaying && !isBuffering && (
          <div
            className="absolute inset-0 flex items-center justify-center z-10 cursor-pointer"
            onClick={handleVideoClick}
          >
            <div className="w-20 h-20 rounded-full bg-black/50 flex items-center justify-center text-white backdrop-blur-sm transition-transform duration-150 hover:scale-110">
          ...
          <>
            ...
            >
              <div className="w-28 h-28 rounded-full bg-black/40 flex items-center justify-center text-white">
                {isPlaying ? <Play className="w-16 h-16 fill-white" /> : <Pause className="w-16 h-16 fill-white" />}
              </div>
            </div> */}
