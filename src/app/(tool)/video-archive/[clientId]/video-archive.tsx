...
const VideoArchive = ({clientId}: {clientId: string}) => {
  ...
  return (
    <div className="dark bg-background min-h-screen text-foreground p-6">
      {clientInfo && (
        <div className="flex items-center gap-4 container  w-fit justify-center">
          {clientInfo.icon && (
      ...
      {!ClientData ? (
        ...
        <div className="flex flex-col gap-10 mt-4 container">
          {[...ClientData].reverse().map((week: ClientDataByWeek, i) => (
            <div key={i} className="border flex flex-col rounded-md pt-3 p-4">
...
`}
    >
      ...
      <span className="whitespace-nowrap overflow-hidden text-ellipsis relative z-20 pointer-events-none w-[250px] font1 mt-1">
        {video.title}
      </span>
