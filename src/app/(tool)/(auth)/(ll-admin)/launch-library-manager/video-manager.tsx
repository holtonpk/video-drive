...
function normalizeVideoData(
  video: Partial<VideoData> | null | undefined,
): VideoData | null {
  if (!video || typeof video !== "object") return null;

  return {
    name: ensureString(video.name),
    cohort: ensureNullableString(video.cohort),
    industry: ensureStringArray(video.industry),
    sector: ensureStringArray(video.sector),
    creativeFormat: ensureStringArray(video.creativeFormat),
    tone: ensureStringArray(video.tone),
    production: ensureStringArray(video.production),
    hook: ensureStringArray(video.hook),
    score: [1, 2, 3, 4, 5].includes(video.score as number)
      ? (video.score as VideoData["score"])
      : null,
    commentary: ensureNullableString(video.commentary),
    description: ensureNullableString(video.description),
    authorUsername: ensureString(video.authorUsername),
    createdAt: ensureString(video.createdAt),
    postId: ensureString(video.postId),
    postUrl: ensureString(video.postUrl),
    likeCount: typeof video.likeCount === "number" ? video.likeCount : 0,
    replyCount: typeof video.replyCount === "number" ? video.replyCount : 0,
    repostCount: typeof video.repostCount === "number" ? video.repostCount : 0,
    viewCount: typeof video.viewCount === "number" ? video.viewCount : 0,
    website: ensureNullableString(video.website),
    ycUrl: ensureNullableString(video.ycUrl),
...
const normalizedLocalSeed: VideoData[] = localData
  .map((item) => normalizeVideoData(item))
...
async function loadLaunchLibraryFromFirebase() {
  ...
  snapshot.forEach((docSnap) => {
    try {
      const raw = docSnap.data() as Partial<VideoData>;
      const data = normalizeVideoData(raw);

      if (!data?.postId) {
        console.error("Invalid video doc: missing postId", docSnap.id, raw);
        return;
      }
      ...
      console.error(
        "Failed to normalize Firebase video doc",
        docSnap.id,
        error,
      );
    }
    ...
    if (this.state.hasError) {
      return (
        <div className="flex min-h-[400px] items-center justify-center p-6">
          <div className="w-full max-w-lg rounded-3xl border border-red-500/30 bg-red-500/10 p-6 text-white">
            <div className="text-lg font-semibold">
              {this.props.fallbackTitle ?? "Something went wrong"}
            </div>
            <p className="mt-2 text-sm text-red-100/80">
              This section failed to render. The error has been logged so you
              can investigate it.
            </p>
            {this.state.errorMessage ? (
              <pre className="mt-4 overflow-auto rounded-xl bg-black/40 p-3 text-xs text-red-200">
                {this.state.errorMessage}
              </pre>
            ) : null}
            <button
...
export default function Page() {
  const [videos, setVideos] = useState<VideoData[]>(normalizedLocalSeed);
  ...
  const [selectedId, setSelectedId] = useState<string | null>(
    normalizedLocalSeed[0]?.postId ?? null,
  );
  ...
  useEffect(() => {
    ...
    async function hydrateFromFirebase() {
      try {
        ...
        setVideos(
          combineLocalAndFirebaseVideos(normalizedLocalSeed, firebaseById),
        );
        setLibraryLoadingState("ready");
      } catch (error) {
        console.error(error);
        if (!isMounted) return;
        setVideos(normalizedLocalSeed);
  ...
  const selectedVideo = useMemo(() => {
    const found = videos.find((video) => video.postId === selectedId);
    return found ? normalizeVideoData(found) : null;
  }, [videos, selectedId]);
...
> = ${JSON.stringify(counts, null, 2)};`;
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-black text-white">
        <h1 className="text-4xl font-bold text-center">
          It&apos;s loading Adam <br /> Just be patient{" "}
        </h1>
      </div>
    );
  }
  ...
  if (libraryLoadingState === "error") {
    return (
      <div className="flex min-h-screen items-center justify-center bg-black text-white">
        <h1 className="text-4xl font-bold text-red-400">Failed to load data</h1>
      </div>
    );
  }
                        ...
                        {selectedVideo &&
                          !videoMatchesFilters(selectedVideo, filters) && (
                            <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
                              ...
                              filters or edit the saved record in the list
                              source.
                            </div>
                          )}
      ...
      >
        <div className="space-y-2">
          {safeOptions.map((option) => {
            const checked = safeValues.includes(option);
