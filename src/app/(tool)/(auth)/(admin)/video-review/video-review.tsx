...
const VideoReview = () => {
  ...
  const orderedVideos = displayVideos?.sort((a, b) => {
      ...
      {loading ? (
        ...
        >
          <div className="flex flex-col w-full h-[calc(100vh-64px)] pb-4 gap-4">
            <h1 className="text-xl text-primary">
              These items need your approval
            </h1>
            {/* <div className="h-fit">
              <FilterStatus
  ...
  return (
    <button
      ...
      className={`border p-4 w-full bg-foreground/40 text-primary rounded-md hover:bg-foreground/50 
        ...
        `}
    >
      <h1 className="w-full overflow-hidden text-ellipsis text-left">
        Review the{" "}
...
const ScriptReview = ({video}: {video: ReviewData}) => {
  ...
  const [isReviewed, setIsReviewed] = React.useState(
    video.videoData.scriptReviewed?.includes(currentUser?.uid) || false
  );
  ...
  return (
    <VideoProvider videoData={video.videoData}>
      <div
        ...
        className="min-w-full w-[200px] h-full rounded-md flex flex-col border p-4 text-primary gap-4 bg-foreground/20 "
      >
        ...
        <div className="h-full flex flex-col max-w-full overflow-hidden gap-4">
          ...
          <div className="grid gap-1">
            <h1 className="font-bold">Reviewed by</h1>
...
const ScriptTab = ({video}: {video: VideoData}) => {
  ...
  const [isReviewed, setIsReviewed] = React.useState(
    video.scriptReviewed?.includes(currentUser?.uid) || false
  );

  useEffect(() => {
    setIsReviewed(video.scriptReviewed?.includes(currentUser?.uid) || false);
  }, [video]);
...
const UploadedVideoReview = ({video}: {video: ReviewData}) => {
  // console.log("video", JSON.stringify(video.script));
  ...
  const [isReviewed, setIsReviewed] = React.useState(
    video.videoData.videoReviewed?.includes(currentUser?.uid) || false
  );
  ...
  const updateVideoField = async (field: string, value: any) => {
    ...
    console.log("vinumn", videoData?.videoNumber);
    try {
      if (videoData?.videoNumber) {
        // Update Firestore document
        console.log("firrrree");
        try {
          await setDoc(
      ...
      // Update local state
      setUploadedVideos((prevVideos: any) => {
        return prevVideos.map((video: any) =>
          video.videoURL === selectedVideo.videoURL
            ? {...video, [field]: value}
            : video
        );
      });

      setSelectedVideo((prevSelected: any) => ({
        ...prevSelected,
        [field]: value,
      }));
    } catch (error) {
  ...
  const markAsReadyToPost = async () => {
    try {
      // Sequentially update both fields
      await updateVideoField("needsRevision", false);
      await updateVideoField("isReadyToPost", true);
  ...
  const markAsNeedsRevision = async () => {
    try {
      // Sequentially update both fields
      await updateVideoField("needsRevision", true);
      await updateVideoField("isReadyToPost", false);
          ...
          <div className="flex flex-col gap-4">
            <div className="flex w-full gap-4">
              <div className="flex flex-col w-fit ">
                <div className="h-[350px] aspect-[9/16] relative rounded-t-md overflow-hidden">
                  <video
              ...
              <div className="w-full border relative h-full bg-foreground/40 rounded-md p-4 flex flex-col gap-2">
                <h1 className="text-xl text-primary">
                  ...
                  <>
                    ...
                    <Textarea
                      ...
                      onChange={(e) => {
                        ...
                        updateVideoField("revisionNotes", e.target.value);
                      }}
                ...
                {!selectedVideo?.needsRevision ? (
                  <>
                    <Button onClick={markAsReadyToPost}>
            ...
            {uploadedVideos && uploadedVideos.length > 0 ? (
              <div className="flex gap-1 flex-col w-full">
                ...
                <div className="flex flex-col gap-1 flex-grow border p-2 rounded-md">
