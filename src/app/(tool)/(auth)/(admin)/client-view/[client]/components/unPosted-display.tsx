...
export const UnPostedDisplay = ({
  ...
  // the status of the video should be done

  const doneVideos = ClientData?.filter((post) => post.status === "done");

  // order ClientData by editing due date then by if its posted
  const orderedClientData = doneVideos?.sort((a, b) => {
  ...
  // the videos where posted is false should be at the top
  const unPostedVideos = orderedClientData?.filter((post) => !post.posted);

  setTotalVideos(unPostedVideos?.length || 0);
    ...
    <>
      {ClientData ? (
        <div className="flex  flex-col gap-6 z-10 relative mx-auto max-w-[1400px] w-full px-8  ">
          ...
          >
            <div
              className={`border bg-card dark:bg-card/40  overflow-auto shadow-lg rounded-md  
                      ${displayedVideo ? "w-full" : "w-[1336px]"}
                      `}
            >
              <div className="w-full p-2 bg-primary/5  dark:bg-muted/60 gap-4 justify-between flex border-b font-bold text-primary text-sm">
                {displayedVideo ? (
                  <>
              ...
              <div className="flex flex-col  dark:bg-transparent">
                {unPostedVideos?.map((post: VideoData, index) => (
                  <VideoColumn
                    key={index}
                    post={post}
                    index={index}
